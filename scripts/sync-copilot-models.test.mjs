...
const FEED = {
  "github-copilot": {
    ...
    models: {
      ...
      "gpt-6-luna": { id: "gpt-6-luna", name: "GPT-6 Luna" },
      "gpt-6-sol": { id: "gpt-6-sol", name: "GPT-6 Sol" },
...
test("buildTable is sorted, non-empty, and includes pinned and catalog entries", () => {
  const table = buildTable(parseCatalog(FEED));
  const ids = table.map((m) => m.id);
  assert.ok(ids.includes("auto"), "auto must be pinned in");
  assert.ok(
    ids.includes("claude-opus-5.5"),
    "catalog Opus model must be available",
  );
  assert.ok(
    ids.includes("gpt-6-luna"),
    "catalog lightweight model must be available",
  );
  assert.ok(
    ids.includes("gpt-6-sol"),
    "catalog powerful model must be available",
  );
  assert.ok(
    ids.includes("claude-opus-4.6"),
    "delisted-but-working model must be retained",
  );
...
test("an empty catalog throws rather than emitting a table", () => {
  assert.throws(
    () => buildTable(parseCatalog({ "github-copilot": { models: {} } })),
    /floor/,
  );
});
...
test("a catalog below the floor throws rather than shrinking the picker", () => {
  const models = {};
  for (let i = 0; i < MIN_CATALOG_MODELS - 1; i++)
    models[`m-${i}`] = { id: `m-${i}` };
  assert.throws(
    () => buildTable(parseCatalog({ "github-copilot": { models } })),
    /floor/,
...
test("priceNameToId slugifies display names to catalog ids", () => {
  ...
  assert.equal(
    priceNameToId("Claude Opus 4.8 (fast mode) (preview)"),
    "claude-opus-4.8",
  );
...
test("reconcile surfaces divergence in explicit buckets, not silent drops", () => {
  ...
  assert.ok(
    rec.catalogNotPriced.includes("grok-4.6"),
    "unpriced catalog model is surfaced, not dropped",
  );
