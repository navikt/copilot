...
3. **Check for internal version skew, not just the dependency's own compatibility.** A common false trail: a version catalog/BOM entry (or an alias) correctly points at a Jackson-3-compatible artifact (e.g. `ktor-serialization-jackson3`), while a *different*, hardcoded version string for the same library elsewhere in the same build file (e.g. `val ktorVersion = "3.3.3"` used for other modules) is still too old for that artifact to exist. This produces an `Unresolved reference` / missing-class error (e.g. `JacksonConverter`, `jacksonObjectMapper`) that looks like a Jackson package-rename mistake but is actually a version mismatch. Before assuming a Jackson-shaped compile error is a migration bug, grep the build file for hardcoded `val xVersion = "..."` assignments used alongside a version catalog/BOM for the same library, and confirm they agree.
...
- Verify framework version minimums before migrating (Ktor 3.4.0+ for `ktor-serialization-jackson3`, Spring Boot 4.0+/Framework 7.0+ for Jackson 3) and check for hardcoded-vs-catalog version skew in the build file
