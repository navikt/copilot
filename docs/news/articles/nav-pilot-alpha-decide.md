...
<!-- UTKAST: publiseres etter nattens måling. Fyll inn alle MÅLT-feltene, fjern draft: true og sjekk datoen. -->
...
tail -n 200 app.log | nav-pilot alpha decide \
  ...
  --options ja,nei --evidence - --threshold 0.7 --expect ja >/dev/null
case $? in
...
grep -v '^#' "$1" | nav-pilot alpha decide \
  "Følger commit-meldingen Conventional Commits?" \
  --options ja,nei --evidence - --threshold 0.8 --expect ja \
...
Hooken stopper bare når modellen faktisk svarte og sannsynligheten for «ja» er under 0,8. Kjører ikke serveren, eller bruker den mer enn tre sekunder, går commiten gjennom. `grep -v '^#'` fjerner kommentarlinjene git legger i meldingsfila.
...
- **Copilot CLI-hooks:** nav-pilot bruker allerede `postToolUse` til loop-guard og maskering, men de to hookene bruker regler, ikke modellen. En egen `postToolUse`-hook får tool-resultatet og kan sende det som grunnlag til `decide`. Hooken må feile åpent og holde seg innenfor tidsgrensen for hooks. Under en økt på den lokale modellen venter `decide` bak økten, så det passer best med en skymodell i økten.
...
- Du kan gi opptil 26 alternativer, men bare de 11 mest sannsynlige tokenene leses av, fordi mlx-lm ikke gir flere. Opptil 11 alternativer er pålitelig.
- Det er en alfa. Flagg og format kan endre seg.

### Målt

<!-- MÅLT: fyll inn fra bench-steget «alpha decide». Ikke publiser før alle fire linjene har tall. -->

- Svartid med varm server: <!-- MÅLT: p50 og p95 i ms -->
- Svartid for første kall etter at serveren er startet: <!-- MÅLT: ms -->
- Treffsikkerhet på commit-meldinger (Conventional Commits): <!-- MÅLT: riktige/antall, snitt-p riktig og galt -->
- Treffsikkerhet på loop eller fremdrift, med tool-resultater som grunnlag: <!-- MÅLT: riktige/antall, snitt-p riktig og galt -->

<!-- MÅLT: én setning om hva tallene betyr for hooks, for eksempel om varm svartid holder for en commit-msg-hook. -->
