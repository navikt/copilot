...
- **To revisjoner beholdes per kilde, pluss dem en økt leser fra:** den pinnede, den den erstattet, og enhver revisjon en levende økt har oppe. Tallet to ble stående der det var. Å heve det løser ingenting: en økt varer i timer og begge klientene slipper flere releaser om dagen, så et hvilket som helst tall er én oppdatering for lite for den økta som varer lengst ([#784](https://github.com/navikt/copilot/issues/784)). Det tredje treet beholdes derfor på bevis i stedet. nav-pilot starter klienten som barneprosess og venter på den, så prosessen som holder treet oppe, er en nav-pilot-prosess som lever nøyaktig like lenge som økta — og en markør med pid og kjernens starttid er det eneste en pruning i en *annen* prosess kan lese. Fortsatt ingen TTL og ingen aldersheuristikk: aldersregelen i `GCStaged` betydde «materialisert for mer enn 24 timer siden», ikke «i bruk», og forskjellen er hele poenget. Dør en økt uten å rydde, holder markøren ingenting — neste pruning ser at prosessen er borte og fjerner den — og starttiden står der fordi et gjenbrukt prosessnummer ellers kunne holdt på et tre for alltid. Diskbruken er dermed to revisjoner per kilde pluss én per distinkt revisjon noen faktisk leser. En lokal kilde følger samme regel: pruningens andre kallsted er install-stien en lokal kilde aldri når, og uninstall går ut fra en state den aldri skriver, så et git-checkout ville ellers lagt igjen en katalog per commit forfatteren starter fra.
