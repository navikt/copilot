...
**Manifestet er autoritativt for personaer.** `PrimaryAgent` leser den aktive pakka og har **ingen** fallback til nav-pilots egen default. En fallback ville injisert Navs nav-pilot-persona inn i en fremmed pakkes launch, samtidig som materialiseringen korrekt degraderer den til subagent (`internal/artifacts/export.go`). Den tomme strengen kan ikke observeres på launch-stiene: schemaet krever `primaryAgents` (`schemas/agentpakke-v1.json`, linje 49) med `"minItems": 1` (linje 56) for hver deklarert klient, sjekket av `agentpakke.Load` før et manifest i det hele tatt festes til en kilde — så den tomme saken er *urepresenterbar*, ikke bare usannsynlig. `stagedPrimaryAgent` feiler likevel høyt framfor å sende et tomt `--agent`, i tilfelle et framtidig kallsted bryter invarianten `SetActivePakke` dokumenterer.
