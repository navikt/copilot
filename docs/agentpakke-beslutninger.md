...
**Beslutnings-id-ene** (`A1`, `D1`, `G3`, `M4` og resten) er definert i [PRD-en, #435](https://github.com/navikt/copilot/issues/435), og delvis gjentatt i [#437](https://github.com/navikt/copilot/issues/437). De siteres i Go-koden, i `DESIGN.md` og i feltbeskrivelsene til det publiserte skjemaet, uten at noen fil i repoet sa hvor de kom fra. Forekomstene i skjemaet er de som betyr mest: det er publisert, så en ekstern pakkeforfatter leser «(A1)» uten noen måte å slå det opp på.
...
Katalogen er permanent og skrives aldri om. En ny revisjon får en ny SHA-katalog, og den forrige blir stående til den ryddes bort ([§6](#6-kjente-begrensninger-vi-har-valgt-med-vilje)). Det eneste unntaket er en lokal kilde, som ikke pinnes og hvis revisjon derfor bygges om hver gang. Det er dette som gjør en fast katalog per pakke riktig nå. Innvendingen mot den var at å tømme og skrive den på nytt trekker config-katalogen ut under en økt som allerede kjører mot den gjennom `OPENCODE_CONFIG_DIR`. For en pinnet, innholdsadressert katalog har den ingenting å treffe ([§8](#8-der-kildene-er-uenige)). For en lokal kilde treffer den, og det ble oversett først: revisjonen bygges om ved hver launch, så en forfatter som starter en ny økt i et annet vindu ville fått katalogen den første leser fra, fjernet under seg. `materializeRevision` sletter derfor ikke den utgående katalogen. Den flytter den til side, publiserer den nye med sin `os.Rename`, og fjerner den gamle først etterpå. Katalogen sees dermed aldri halvslettet: den er enten den gamle revisjonen, kort borte mellom de to renamene, eller den nye. Den kjørende økten beholder deskriptorene sine i treet som ble flyttet, og leser videre. Fikset i #504 (U9).
...
**Fail-closed begynner ved tier-porten, ikke før den.** Dette gjelder uendret, og det gjelder nå de launchene som faktisk må resolve: en upinnet kilde, en Tier 1-kilde, en kilde uten manifest. En pinnet launch resolver ikke og møter aldri porten. Ellers resolves kilden, fordi det er slik nav-pilot får vite om denne launchen i det hele tatt er Tier 2. En resolve-feil lander *før* tier-porten, der ingenting ennå sier at det er en payload i bildet, siden kilden kanskje ikke deklarerer noen. Å være offline, eller ha et utdatert reponavn i config, skal derfor ikke blokkere en launch som virket før Tier 2-staging fantes. nav-pilot advarer og tar legacy-stien, slik `EnsureOpenCodeNavContext` alltid har gjort. **Unntaket er når nav-pilot allerede vet svaret.** Pin-staten noterer per klient hvilke klienter den pinnede revisjonen materialiserte payloads for (`pinned_clients`, skrevet av `install`, `sync --apply` og auto-pinnen), og tier-cachen kan huske et `TierPayload`-svar fra en nylig resolve. Sier en av dem at kilden har staget payload for denne klienten, er «ingenting sier ennå at det er Tier 2» ikke sant, og en legacy-launch der er den samme stille nedgraderingen som over: brukerens eget `~/.copilot` blandet inn i en launch manifestet reserverer for et verifisert payload-tre, bak en stderr-advarsel klientens TUI visker bort. Den launchen nekter i stedet, og navngir `nav-pilot sync --apply` og `nav-pilot config set source ""`. Pin-staten svarer først og har ingen TTL. Cachen alene lot tre vanlige offline-stier nedgradere stille (#544): en pin slettet for hånd etter at cache-oppføringa var utløpt, en kilde installert med `install` som aldri launch-resolvet — install skriver ikke cachen — og klient nummer to på en kilde bare den første klienten hadde resolvet. Nektelsen er per klient, fordi pinnen er per kilde mens payloads er per klient: en klient pinnen aldri staget noe for, tar legacy-stien som før. En pin skrevet før `pinned_clients` fantes kan ikke skille de to offline; den nekter, og oppgraderes på plass ved første launch eller sync som får lest det pinnede manifestet. Fikset i #504 (U8) og #544. Forbi tier-porten, der payloaden er kjent, er alt fail-closed uten fallback.
...
**Tier-cachen blir stående, innsnevret og dokumentert ([#469](https://github.com/navikt/copilot/issues/469)).** `tierCacheTTL` (6 timer, `internal/cli/pakke_launch.go`) finnes for å slippe en `git clone --depth 1` ved hver launch bare for å lære en tier. Etter pinnen står den igjen med nøyaktig én sti: en egen kilde som resolver til Tier 1, eller som ikke har noe manifest. `tryPakkeLaunch` kjører fortsatt for den, fordi porten øverst i funksjonen bare stopper launcher uten kilde satt, og uten cachen ville den klonet ved hver eneste launch. Det gjelder også `source = "navikt/copilot"` skrevet eksplisitt, som `persistInstalledSource` skriver etter enhver `install --source`. En pinnet kilde treffer ikke cachen, fordi pinnen svarer før den konsulteres, og en upinnet payload-only kilde resolver uansett én gang for å bli pinnet. **Det bærende svaret er ikke-payload-svaret**, som sparer klonen. Som offline Tier 2-signal er cachen avløst av pin-statens `pinned_clients` ([#544](https://github.com/navikt/copilot/issues/544)), som er durabel og per klient; et husket `TierPayload`-svar leses nå bare som sekundært signal, for en payload-kilde som ble resolvet uten å bli pinnet — en blandet pakke, eller en auto-pin som nektet over en fremmed installasjon.
...
Vi planlegger å assertere config-plassering, persona inkludert `<pakke>:<agent>`-kvalifiseringen, modellhåndtering, `--pass-env OPENCODE_CONFIG_DIR`, `--allow-read <staged>` og cplt-agentvalg. Vi tar i bruk `--no-audit` og utelater fortsatt `--project-dir` ([§2.5](#25-flagg-fra-referansen)), normaliserer modes ved staging ([§2.1](#21-modesjekk-på-kilden-subset-pluss-exec-bit)), og speiler `.gitignore`-pre-seedingen etter [#565](https://github.com/navikt/copilot/issues/565) ([§2.6](#26-annet-vi-ikke-speiler)). Spørsmålet til eSyfo var om noen av dem er bærende for deres guardrail-historie. **Besvart for flaggene:** `--no-audit` er bærende og tas i bruk, `--project-dir` kan utelates så lenge arbeidskatalogen er prosjektomfanget ([kommentar i #437](https://github.com/navikt/copilot/issues/437#issuecomment-5437575432)). Resten av G4-ordlyden er fortsatt stilt samme sted som 5.1.
...
- **To revisjoner beholdes per kilde, pluss den en økt leser fra:** den pinnede, den den erstattet, og enhver revisjon en levende økt har oppe. Tallet to ble stående der det var. Å heve det løser ingenting: en økt varer i timer og begge klientene slipper flere releaser om dagen, så et hvilket som helst tall er én oppdatering for lite for den økta som varer lengst ([#784](https://github.com/navikt/copilot/issues/784)). Det tredje treet beholdes derfor på bevis i stedet. nav-pilot starter klienten som barneprosess og venter på den, så prosessen som holder treet oppe, lever nøyaktig like lenge som økta — og beviset er en markørfil den prosessen holder en `flock` på. Kjernen slipper den låsen når prosessen dør, så en markør pruningen får låse selv, er per definisjon en markør uten økt bak seg: ingen pid å tro på, ingen `ps` som må svare, ingenting som utløper på alder. Fortsatt ingen TTL og ingen aldersheuristikk — aldersregelen i `GCStaged` betydde «materialisert for mer enn 24 timer siden», ikke «i bruk», og forskjellen er hele poenget. Kravet tas i det en revisjon navngis, ikke når klienten starter, og publisering og pruning utelukker hverandre med en `flock` på kildekatalogen; en pruning som ikke får låsen, eller som ikke får lest markørene, sletter ingenting. Diskbruken er dermed to revisjoner per kilde pluss én per levende økt. En lokal kilde publiserer hver materialisering under sitt eget katalognavn, fordi den samme commiten stagd på nytt er et nytt tre: katalognavnet er identiteten, og uten det ville en markør navngitt et tre som var byttet ut under økta. Ellers følger den samme regel — pruningens andre kallsted er install-stien en lokal kilde aldri når, og uninstall går ut fra en state den aldri skriver, så et git-checkout ville ellers lagt igjen en katalog per commit forfatteren starter fra.
...
- **En probe som feiler sier hvilken av to ting som gikk galt.** «Ikke installert» og «installert, men startet ikke» kom begge tilbake som `exit status 1` bak «could not read the version». Klientens egen stderr ble samlet opp av `exec.Cmd.Output` og aldri lest — #565s OpenCode skrev «Error: Unexpected server error» nettopp der. `probeFailure` (`internal/provider/runtime_gate.go`) dekker alt `exec.Cmd.Output` kan gi: ingenting å kjøre, en prosess som kjørte og avsluttet med en ikke-null exit-kode (meldingen tar med koden og første stderr-linje), en fil som ikke lot seg gjøre om til en prosess i det hele tatt (EACCES, ENOEXEC, EISDIR — verken exit-kode eller stderr, bare OS-årsaken), og en probe som startet, avsluttet med 0, men aldri leverte utdataene sine fordi noe den startet fortsatt holder utdatarøret. Det siste er `exec.ErrWaitDelay` og er ikke en startfeil; den ble først meldt som en. ENOENT skiller ikke i seg selv «finnes ikke» fra «finnes, men mangler tolkeren eller lasteren sin» — `execve` svarer det samme i begge tilfellene — så stien stat-es før klienten får skylda.
