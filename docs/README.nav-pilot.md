...
0 når du er à jour. Vil du ha en bestemt versjon, bruk pakkebehandleren eller last den ned fra
[releasene](https://github.com/navikt/copilot/releases?q=nav-pilot).

Med `auto_update = true` oppgraderer nav-pilot seg selv før andre kommandoer, med fremdriften på
stderr, så `--json` på stdout forblir gyldig. Feiler oppgraderingen, får du en advarsel, kommandoen
kjører på versjonen du har, og neste forsøk kommer etter 24 timer. Slå det av med
`nav-pilot config set auto_update false`.
