# Video Series Article Summaries

Short, actionable article summaries for each video episode (displayed on video detail pages).
...
Når du reviewer mange PRs, kjører du gjerne kodegjennomgang som separate Copilot-sesjoner. Det betyr hver review starter fra topp—full kontekst-overhead for hver PR. I denne videoen lærer du å samle reviews: batch 5-6 PRs i én sessjon, bruk `/diff` for å laste inn filendringer uten å paste dem manuelt, og la Copilot samenligne på tvers av filene.

Resultatet: 70% færre tokens per review, raskere feedback, og færre misforståelser mellom reviewerne. Vi viser konkret workflow: last diff, angi fokusregler ("les for sikkerhet"), få kommentarer alle PR-er samtidig. Når du har 10+ PRs i backlog, er denne teknikken en game-changer.
...
Å sette opp Copilot lokalt kan virke komplisert: installation, konfiguration, testing. I denne videoen viser vi Cplt sandbox—en pre-konfigurert testmiljø som gir deg en arbeidsende Copilot-setup på 3 minutter, med alt fra integrasjoner til eksempler allerede på plass.

Bruk sandboxen til å eksperimentere med nye workflows, teste optimiseringsteknikker, eller lære agentdelegeringen uten å påvirke din normale setup. Det er som en sikker øvingsgrund for Copilot-bruken din. Etter denne videoen kan du starte experimenter med det samme.

---

## Bonus Episode E: rtk — CLI-output-komprimering (60-90% token-besparelse)

Hver gang du kjører `go test -v` fra terminalen, scrolles du gjennom hundrevis av linjer. Mye støy, lite signal. Med `rtk` foran kommandoen får du det samme resultatet på én linje—og 95% færre tokens. rtk er en filter som sitter foran alle dine CLI-kommandoer og klipper vekk støyen automatisk.

Ingen oppsetting, ingen læringskurve. Bare legg `rtk` foran hver kommando og se resultatene med `rtk gain`. Vi viser fem konkrete eksempler: git log, go test, git diff, docker ps, og gh CLI—hver med 60-90% besparelse. Prøv rtk foran din neste kommando. Du merker resultatet med en gang.

---

## Usage Notes

- **JSON format** (`video-articles.json`): Use for programmatic loading on video detail pages
- **Markdown format** (`video-articles.md`): Use for easy reading, reference, and editing
- **Tone**: Concrete problems first, specific payoffs with numbers, actionable ending
- **Length**: 2 paragraphs per episode, ~150-200 words each
- **Standalone**: Each summary works without seeing other videos
