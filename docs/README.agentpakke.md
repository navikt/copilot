...
- **Én enkelt artefakt.** `nav-pilot install <agentnavn> --frozen` — et navn som ikke er en samling, men én agent eller ferdighet — nektes. En a-la-carte-install skriver ingen erklæring og installerer det ene navnet uansett hva `items` sier, så det finnes ikke noe der å holde den til. `--type` er den samme installasjonen bedt om eksplisitt, og avvises som bruksfeil på flaggnivå (exit `1`) med den samme sjekken liggende under, for at flaggsjekken skal være en beleilighet framfor det eneste som står mellom `--frozen` og en helt usjekket installasjon.
- **En Tier 2-agentpakke.** `--frozen` dekker dem ikke, og sier det. En Tier 2-install pinner én revisjon per *bruker* og digest-verifiserer hvert payload-tre mot manifestet sitt: den er allerede frossen i konstruksjonen, og det finnes ingen repo-scopet Tier 2-install for erklæringa å holde den til (`guardPakkeScope` nekter alle andre scope enn brukerens). Meldinga ber deg installere den én gang *uten* `--frozen`, framfor å be om `--user` — et flagg `--frozen` selv avviser.

Flagg som strekker seg forbi erklæringa, motsier `--frozen` og avvises som bruksfeil (exit `1`) framfor å vinne eller tape i stillhet: `--source`, `--ref`, `--type` og `--user` (bruker-scope har ikke noe repo og leser aldri erklæringa). `--user` er én bruksfeil med ett svar: både flaggsjekken og sjekken under den svarer `1`. `--dry-run` og `--json` er forenlige — nektinga skjer før JSON-dokumentet skrives, så den maskinlesbare veien heller ikke kan melde en halv install som suksess. Merk at `--dry-run --frozen` selv nekter med `3` når arbeidstreet allerede avviker fra den pinnede revisjonen: konfliktsjekken kjører før tørrkjøringa, og en tørrkjøring som svarte «ville installert» over et tre den ekte installasjonen nekter, ville vært verre enn ingen tørrkjøring. `nav-pilot install --frozen` uten navn åpner ingen velger; den ber om navnet, slik at kommandolinja selv sier hva den forventer å installere.
