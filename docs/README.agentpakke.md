...
- **Pinnen flyttes av `nav-pilot sync`, ikke av å starte klienten på nytt.** Unntaket er når du svarer ja på spørsmålet om en ny stabil release ved oppstart ([Ved oppstart](#ved-oppstart)). Uten `--apply` rapporterer sync hvilken revisjon som er tilgjengelig. Med `--apply` verifiseres og materialiseres den nye revisjonen, og pinnen bytter. De to siste revisjonene beholdes, og i tillegg dem en økt leser fra akkurat nå (neste punkt); resten fjernes. Er den pinnede revisjonen fjernet fra disk, sier sync det framfor å melde «up to date», og `--apply` bygger den opp igjen (`Restored …`). Sync oppdaterer den kilden scopet er pinnet til: peker du den mot et annet repo, nekter den og ber deg gjøre byttet med `install`.
- **En revisjon en økt leser fra, fjernes ikke.** Klienten leser katalogen hele økta — `OPENCODE_CONFIG_DIR`, `--plugin-dir`, cplt-ens `--allow-read` — og nav-pilot starter klienten som barneprosess og venter på den. Ved launch skrives derfor en markør under `~/.nav-pilot/pakker/<kilde>/.i-bruk/` med pid-en og kjernens starttid for den, og den fjernes når økta er over. Pruningen beholder revisjonene markørene navngir, i tillegg til de to siste, så to oppdateringer midt i en økt tar ikke treet under den ([#784](https://github.com/navikt/copilot/issues/784)). Dør økta uten å rydde, holder markøren ingenting: neste pruning ser at prosessen er borte, og fjerner både markøren og revisjonen. Starttiden står der fordi prosessnumre gjenbrukes — uten den kunne en død økt holde på et tre for alltid.
...
- **Utvikler du pakka lokalt, pinnes den ikke.** Er `source` en absolutt sti (`nav-pilot config set source /sti/til/pakke`), materialiseres og verifiseres payloadene på nytt ved hver launch, så en endring i arbeidstreet er med på neste start. Samme oppbevaringsregel gjelder: revisjonen launchen bruker, den forrige, og dem en økt leser fra. `nav-pilot install` av en lokal Tier 2-kilde nektes: det finnes ingen immutabel revisjon å pinne, og meldingen navngir install fra repoet i stedet. Dette er flyten for å utvikle en pakke.
