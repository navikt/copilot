...
Dette dokumentet er kontrakten, og det eneste stedet den står. Skal du lage din første pakke, følg oppskrifta på [ki-utvikling.nav.no/nav-pilot/agentpakker](https://ki-utvikling.nav.no/nav-pilot/agentpakker) og slå opp her underveis. Interndesignet, altså hvordan manifestet trådes gjennom install og sync, står i [cli/nav-pilot/DESIGN.md](../cli/nav-pilot/DESIGN.md).
...
Oppdagelsen feiler lukket: en kodesøkfeil eller et tomt svar feiler jobben, fordi «ingenting å validere» og «fikk ikke sett etter» ellers ser like ut. Struping er unntaket, for den sier ingenting om endringen som vurderes: søket prøves på nytt så lenge API-et ber om, med tak på fem minutter venting til sammen, og en kjøring som fortsatt er strupet sveiper de kjente pakkene og advarer i stedet for å feile — men da må hver av dem validere, så en kjent pakke som ikke lar seg klone feiler jobben. En pakke som ellers ikke lar seg klone meldes som advarsel og blokkerer ingen merge — en utilgjengelig pakke sier ingenting om kontrakten. Sveipet har et tak på 25 pakker og navngir det som faller utenfor. Det erstatter ikke pakkerepoets egen CI: sveipet fanger at kontrakten brøt en pakke, mens pakkerepoets egen jobb under fanger at pakka har driftet fra kontrakten.
...
- **Pinnen flyttes av `nav-pilot sync`, ikke av å starte klienten på nytt.** Unntaket er når du svarer ja på spørsmålet om en ny stabil release ved oppstart ([Ved oppstart](#ved-oppstart)). Uten `--apply` rapporterer sync hvilken revisjon som er tilgjengelig. Med `--apply` verifiseres og materialiseres den nye revisjonen, og pinnen bytter. De to siste revisjonene beholdes, og i tillegg den revisjonen en økt leser fra akkurat nå (neste punkt); resten fjernes. Er den pinnede revisjonen fjernet fra disk, sier sync det framfor å melde «up to date», og `--apply` bygger den opp igjen (`Restored …`). Gikk den nye revisjonen i stykker, tar `nav-pilot rollback` deg tilbake til den forrige uten nett, så lenge den fortsatt ligger der ([Rollback uten nett](#rollback-uten-nett)). Sync oppdaterer den kilden scopet er pinnet til: peker du den mot et annet repo, nekter den og ber deg gjøre byttet med `install`.
- **En revisjon en økt leser fra, fjernes ikke.** Klienten leser katalogen hele økta — `OPENCODE_CONFIG_DIR`, `--plugin-dir`, cplt-ens `--allow-read` — og nav-pilot starter klienten som barneprosess og venter på den. Så snart en launch har navngitt en revisjon, tar den derfor en markørfil under `~/.nav-pilot/pakker/<kilde>/.i-bruk/` og holder en `flock` på den så lenge økta varer. Pruningen beholder revisjonene låste markører navngir, i tillegg til de to siste, så to oppdateringer midt i en økt tar ikke treet under den ([#784](https://github.com/navikt/copilot/issues/784)). Dør økta uten å rydde, slipper kjernen låsen: neste pruning får låse markøren selv, og fjerner både den og revisjonen. Ingenting utløper på alder, og ingen pid slås opp — en markør pruningen får låse, er per definisjon en markør uten økt bak seg.
- **Publisering av en markør og pruning utelukker hverandre.** Begge tar en `flock` på kildekatalogen, så pruningen aldri leser et sett markører som er under endring. En pruning som ikke får låsen, eller som ikke får lest `.i-bruk`, sletter ingenting: «ingen markører kan leses» og «ingen økter kjører» ser like ut og betyr motsatte ting.
- **`nav-pilot uninstall` fjerner bare filene nav-pilot fortsatt eier.** En fil som er endret siden nav-pilot skrev den, blir liggende og navngis (`differs from what nav-pilot installed, kept`), sammen med en linje som sier hvor mange. Det er samme regel som styrer sletting i sync, og `nav-pilot uninstall --force` er veien forbi den. Revisjonene under `~/.nav-pilot/pakker/` er ikke installerte filer og tas uansett.
...
- Taggen binder versjonen: fjern et valgfritt `<prefiks>/` og en valgfri `v`, og resten skal være lik `version`. Et prefiks må være pakkenavnet. For grillmester binder `v0.4.1`, `0.4.1` og `grillmester/v0.4.1` alle til `0.4.1`, mens `annen/v0.4.1` og en tag som ikke er en versjon (for eksempel `latest`) hoppes over uten at assetet lastes ned.
...
Starter du klienten i en terminal fra en pinnet pakke i brukerscope, og det finnes en nyere stabil release, spør nav-pilot før klienten starter:
...
- **keep** stopper både oppslaget og spørsmålet ved oppstart — en launch under valget rører ikke nettet — og `sync` flytter ikke pinnen, heller ikke med `--apply`. Beskjeden stoppes ikke: `sync` og `nav-pilot list --installed` navngir fortsatt det som holdes tilbake — en release, eller revisjonen på standardgrenen for en kilde uten releases — og hva som skal til for å ta det. Det gjelder også en sikkerhetsrettelse: den kommer når du sier fra, som er hele meningen med valget.
- `--updates` skrives før synken som følger, så `sync --user --apply --updates auto` er én setning: ta nye releases fra nå av, denne inkludert. Et ukjent modusnavn avvises, og staten står. Uten pinne i brukerscope sier kommandoen det. Navngir kallet et annet scope (`--repo`, `--target`), avvises det: valget gjelder pinnen i brukerscope, og et kall som ber om noe annet, blir ikke stille omdirigert dit.
...
Går en ny revisjon i stykker, flytter `nav-pilot rollback` pinnen tilbake til forrige revisjon på maskinen. Den leser bare disk: ingen kloning, ingen release-oppslag, ingenting som krever nett.

```bash
nav-pilot rollback          # forrige revisjon på denne maskinen
nav-pilot rollback --json   # command, collection, scope, source_sha, rolled_back_from, follows_releases
```
