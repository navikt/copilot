...
func Main(info BuildInfo) {
	...
	// The profile rides along with the release check rather than getting a
	// budget of its own: artifacts only calls this when the release lookup it
	// just made succeeded, inside the same TTL and the same 5 seconds.
	artifacts.FetchProfile = func() ([]byte, error) {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
