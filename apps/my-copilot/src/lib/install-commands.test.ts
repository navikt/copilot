...
const remoteSseMcp: McpServerCustomization = {
  ...
  type: "mcp",
  name: "github-mcp-sse",
  serverId: "io.github.navikt/github-mcp-sse",
  version: "1.0.0",
...
describe("getMcpServerConfig", () => {
  ...
  it("generates sse config for sse remote mcp", () => {
    ...
    expect(config).toEqual({
      "io.github.navikt/github-mcp-sse": { type: "sse", url: "https://mcp.nav.no/sse" },
    });
  });
...
describe("getVsCodeAddMcpCommand", () => {
  ...
  it("generates code --add-mcp with sse type for sse remote", () => {
    ...
    const json = JSON.parse(cmd.replace("code --add-mcp '", "").replace(/'$/, ""));
    expect(json.name).toBe("io.github.navikt/github-mcp-sse");
